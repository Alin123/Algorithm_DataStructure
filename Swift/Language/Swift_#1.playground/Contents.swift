...
//MARK: Int/UInt 长度与当前平台的原生字长相同
let fourByteIn32BitOS: Int = Int(Int32.max)
let eightByteIn64BitOS: Int = Int(Int64.max)

//MARK: 浮点数
//MARK: Double表示64位浮点数，可存储很大或者很高精度的浮点数。精度很高，至少有15位数字。
//MARK: Float表示32位浮点数，可存储精度要求不高的浮点数。只有6位数字。
...
//MARK: 可选类型Optionals表示有值等于x或者没有值
let possibleNumber = "123"
let convertedNumber = Int(possibleNumber)
type(of: convertedNumber)

//MARK: nil，可以给可选变量赋值为nil表示它没有值，但不能用于非可选的常量和变量。
var surveyAnswer: String?

//MARK: if语句及强制解析，可以使用if语句和nil比较来判断一个可选值是否包含值。
if convertedNumber != nil {
    print("converteNumber contains some integer value.")
}

//MARK: 解包，当确定可选类型确实包含值之后，可以在可选的名字后加一个感叹号！来获取值。表示“我知道这个可选有值，请使用它”
if convertedNumber != nil {
    print("convertedNumber has an integer value of \(convertedNumber!).")
}

//MARK: 可选绑定optional binding
...
//MARK: 隐式解析可选类型，其实就是一个普通的可选类型，但是可以被当做非可选类型来使用，并不需要每次都使用解析来获取可选值。如果一个变量之后可能变成nil的话请不要使用隐式解析可选类型。如果你需要在变量的生命周期中判断是否是nil的话，请使用普通可选类型。可以把隐式解析可选类型当做一个可以自动解析的可选类型。如果在隐式解析可选类型没有值的时候尝试取值，会触发运行时错误。和你在没有值的普通可选类型后面加一个惊叹号一样。
...
//MARK: 断言，用它来检查在执行后续代码之前是否一个必要的条件已经被满足了。
let age = 3
assert(age >= 0, "A person's age connot be less than zero!")

//MARK: 先决条件，当一个条件可能为false(假)，但是继续执行代码要求条件必须为true(真)的时候，需要使用先决条件。
let idx = 1
...
//MARK: 空合运算符Nil Coalescing Operator，空合运算符(a ?? b)将对可选类型a进行空判断，如果a包含一个值就进行解封，否则就返回一个默认值b。
...
//MARK: 比较字符串：字符串/字符相等、前缀/后缀相等

//MARK: 字符串Unicode表示形式
// http://www.swift51.com/swift4.0/chapter2/03_Strings_and_Characters.html#unicode_representations_of_strings
...
//MARK: switch语句中的break。swift所有的分支中不允许为空的分支，有时为了使意图明确，在想忽略的分支上加break语句。当这个分支被匹配到时，分支内的break语句立即结束switch代码块。
//MARK: 带标签的语句
square = 0
print("game start!")
